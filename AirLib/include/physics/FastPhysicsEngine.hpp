...
private:
    ...
    static void updateCollisonResponseInfo(const CollisionInfo& collison_info, const Kinematics::State& next, bool is_collison_response, CollisonResponseInfo& collison_response_info)
    {
        collison_response_info.collision_time_stamp = collison_info.time_stamp;
        ++collison_response_info.collison_count_raw;

        //increment counter if we didn't collided with high velocity (like resting on ground)
        if (is_collison_response && next.twist.linear.squaredNorm() > kRestingVelocityMax * kRestingVelocityMax)
            ++collison_response_info.collison_count_non_resting;
        ...
        const Kinematics::State& current, Kinematics::State& next, Wrench& next_wrench, bool enable_ground_lock)
    {
        ...
        //are we going away from collison? if so then keep using computed next state
        if (collison_info.normal.dot(next.twist.linear) >= 0.0f)
            return false;
